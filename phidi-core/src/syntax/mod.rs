        ...
        let injection_callback = |language: &InjectionLanguageMarker| {
            let language = match language {
                InjectionLanguageMarker::Name(name) => {
                    PhidiLanguage::from_name(name)
                }
                InjectionLanguageMarker::Filename(path) => {
                    PhidiLanguage::from_path_raw(path)
                }
                InjectionLanguageMarker::Shebang(id) => PhidiLanguage::from_name(id),
            };
            language
...
impl Syntax {
    pub fn init(path: &Path) -> Syntax {
        let language = PhidiLanguage::from_path(path);
        Syntax::from_language(language)
    }

    pub fn plaintext() -> Syntax {
        Self::from_language(PhidiLanguage::PlainText)
    }

    pub fn from_language(language: PhidiLanguage) -> Syntax {
    ...
    pub fn find_enclosing_pair(&self, offset: usize) -> Option<(usize, usize)> {
        if self.language == PhidiLanguage::Markdown {
