...
use phidi_rpc::style::{LineStyle, Style};
use regex::Regex;
...
/// indices into the LANGUAGES array.  See method `PhidiLanguage::properties`.
///
...
/// `PhidiLanguage` as they will be accessed using the enum variants as indices.
const LANGUAGES: &[SyntaxProperties] = &[
    // Undetected/unmatched fallback or just plain file
    SyntaxProperties {
        id: PhidiLanguage::PlainText,
        indent: Indent::tab(),
        files: &[],
        extensions: &["txt"],
    ...
    SyntaxProperties {
        id: PhidiLanguage::Gitattributes,
        indent: Indent::tab(),
        files: &[],
        extensions: &[],
        comment: comment_properties!(),
    ...
    SyntaxProperties {
        id: PhidiLanguage::KotlinBuildScript,
        indent: Indent::space(2),
        files: &[],
        extensions: &["kts"],
        comment: CommentProperties {
    ...
    SyntaxProperties {
        id: PhidiLanguage::MarkdownInline,
        indent: Indent::space(4),
    ...
    SyntaxProperties {
        id: PhidiLanguage::OcamlInterface,
        indent: Indent::space(2),
        files: &[],
        extensions: &["mli"],
        comment: comment_properties!("(*"),
    ...
    SyntaxProperties {
        id: PhidiLanguage::SshClientConfig,
        indent: Indent::tab(),
        files: &[],
        extensions: &[],
        comment: comment_properties!(),
...
impl PhidiLanguage {
    ...
    pub fn from_path(path: &Path) -> PhidiLanguage {
        Self::from_path_raw(path).unwrap_or(PhidiLanguage::PlainText)
    }

    pub fn from_path_raw(path: &Path) -> Option<PhidiLanguage> {
    ...
    pub fn from_name(name: &str) -> Option<PhidiLanguage> {
        match PhidiLanguage::from_str(name.to_lowercase().as_str()) {
            Ok(v) => Some(v),
            Err(e) => {
                event!(Level::DEBUG, "failed parsing `{name}` PhidiLanguage: {e}");
                None
            }
        }
    }
    ...
    fn get_grammar(&self) -> Option<tree_sitter::Language> {
        ...
        if let Some(grammars_dir) = Directory::grammars_directory() {
            match self::load_grammar(&grammar_name, &grammar_fn_name, &grammars_dir)
            {
                ...
                Err(err) => {
                    if self != &PhidiLanguage::PlainText {
...
mod tests {
    ...
    use super::PhidiLanguage;

    #[test]
    fn test_lanaguage_from_path() {
        let l = PhidiLanguage::from_path(&PathBuf::new().join("test.rs"));
        assert_eq!(l, PhidiLanguage::Rust);
    }
}
