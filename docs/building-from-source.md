...
It is easy to build Phidi from source on a GNU/Linux distribution. Cargo handles the build process, all you need to do, is ensure the correct dependencies are installed.
...
```sh
git clone https://github.com/phidi/phidi.git ~/phidi
```
...
```sh
cargo install --path . --bin phidi --profile release-lto --locked
```

> If you use a different distribution, and are having trouble finding appropriate dependencies, let us know in an issue!

Once Phidi is compiled, the executable will be available in `$HOME/.cargo/bin/phidi` and should be available in `PATH` automatically.
...
Additionally to building multiple OS versions at the same time, Docker-based builds will also try to cross-compile Phidi for other architectures.
