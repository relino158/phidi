...
use phidi_core::{language::PhidiLanguage, syntax::Syntax};
use phidi_xi_rope::Rope;
use pulldown_cmark::{CodeBlockKind, CowStr, Event, Options, Parser, Tag};
use smallvec::SmallVec;

use crate::config::{PhidiConfig, color::PhidiColor};
                        ...
                        Tag::CodeBlock(kind) => {
                            let language =
                                if let CodeBlockKind::Fenced(language) = kind {
                                    md_language_to_phidi_language(language)
                                } else {
                                    None
                                };
            ...
            Event::Html(text) => {
                attr_list.add_span(
                    ...
                    default_attrs
                        ...
                        .color(config.color(PhidiColor::MARKDOWN_BLOCKQUOTE)),
                );
                current_text.push_str(&text);
                pos += text.len();
                builder_dirty = true;
...
fn attribute_for_tag<'a>(
    ...
    match tag {
        ...
        Tag::Link {
            ...
            // TODO: Link support
            Some(default_attrs.color(config.color(PhidiColor::EDITOR_LINK)))
        }
...
fn md_language_to_phidi_language(lang: &str) -> Option<PhidiLanguage> {
    // TODO: There are many other names commonly used that should be supported
    PhidiLanguage::from_name(lang)
}
