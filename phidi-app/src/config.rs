...
use phidi_proxy::plugin::wasi::find_all_volts;
...
use crate::workspace::{PhidiWorkspace, PhidiWorkspaceType};
...
static DEFAULT_CONFIG: Lazy<config::Config> = Lazy::new(PhidiConfig::default_config);
static DEFAULT_PHIDI_CONFIG: Lazy<PhidiConfig> =
    Lazy::new(PhidiConfig::default_phidi_config);
...
static DEFAULT_DARK_THEME_COLOR_CONFIG: Lazy<ColorThemeConfig> = Lazy::new(|| {
    let (_, theme) =
        PhidiConfig::load_color_theme_from_str(DEFAULT_DARK_THEME).unwrap();
...
impl PhidiConfig {
    pub fn load(
        ...
        let mut phidi_config: PhidiConfig = match config.try_deserialize() {
            Ok(config) => config,
            Err(error) => {
                error!("Failed to deserialize configuration file: {error}");
                DEFAULT_PHIDI_CONFIG.clone()
            }
        };

        phidi_config.available_color_themes =
            Self::load_color_themes(disabled_volts, extra_plugin_paths);
        phidi_config.available_icon_themes =
            Self::load_icon_themes(disabled_volts, extra_plugin_paths);
        phidi_config.resolve_theme(workspace);

        phidi_config.color_theme_list = phidi_config
            .available_color_themes
            .values()
            .map(|(name, _)| name.clone())
            .sorted()
            .collect();
        phidi_config.color_theme_list.sort();

        phidi_config.icon_theme_list = phidi_config
            .available_icon_themes
            .values()
            .map(|(name, _, _)| name.clone())
            .sorted()
            .collect();
        phidi_config.icon_theme_list.sort();

        phidi_config.wrap_style_list = im::vector![
        ...
        phidi_config.terminal.get_indexed_colors();
        ...
        match workspace.kind {
            PhidiWorkspaceType::Local => {
                if let Some(path) = workspace.path.as_ref() {
                    let path = path.join("./.phidi/settings.toml");
                    config = config::Config::builder()
                        .add_source(config.clone())
                        .add_source(
            ...
            PhidiWorkspaceType::RemoteSSH(_) => {}
            #[cfg(windows)]
            PhidiWorkspaceType::RemoteWSL(_) => {}
        }
    ...
    fn default_phidi_config() -> PhidiConfig {
        let mut default_phidi_config: PhidiConfig =
        ...
        default_phidi_config.color_theme = DEFAULT_DARK_THEME_COLOR_CONFIG.clone();
        default_phidi_config.icon_theme = DEFAULT_ICON_THEME_ICON_CONFIG.clone();
        default_phidi_config.resolve_colors(None);
        default_phidi_config
    }

    fn resolve_theme(&mut self, workspace: &PhidiWorkspace) {
        let default_phidi_config = DEFAULT_PHIDI_CONFIG.clone();
        ...
        self.resolve_colors(Some(&default_phidi_config));
        self.update_id();
    }
    ...
    pub fn set_color_theme(&mut self, workspace: &PhidiWorkspace, theme: &str) {
    ...
    /// Note that this does not save the config.
    pub fn set_icon_theme(&mut self, workspace: &PhidiWorkspace, theme: &str) {
        self.core.icon_theme = theme.to_string();
        self.resolve_theme(workspace);
    }

    pub fn set_modal(&mut self, _workspace: &PhidiWorkspace, modal: bool) {
    ...
    fn resolve_colors(&mut self, default_config: Option<&PhidiConfig>) {
        ...
        let fg = self.color(PhidiColor::EDITOR_FOREGROUND).to_rgba8();
        let bg = self.color(PhidiColor::EDITOR_BACKGROUND).to_rgba8();
    ...
    pub fn files_svg(&self, paths: &[&Path]) -> (String, Option<Color>) {
        ...
        if let Some(svg) = svg {
            let color = if self.icon_theme.use_editor_color.unwrap_or(false) {
                Some(self.color(PhidiColor::PHIDI_ICON_ACTIVE))
            } else {
                None
            };
            (svg, color)
    ...
    pub fn symbol_svg(&self, kind: &SymbolKind) -> Option<String> {
        let kind_str = match *kind {
            SymbolKind::ARRAY => PhidiIcons::SYMBOL_KIND_ARRAY,
            SymbolKind::BOOLEAN => PhidiIcons::SYMBOL_KIND_BOOLEAN,
            SymbolKind::CLASS => PhidiIcons::SYMBOL_KIND_CLASS,
            SymbolKind::CONSTANT => PhidiIcons::SYMBOL_KIND_CONSTANT,
            SymbolKind::ENUM_MEMBER => PhidiIcons::SYMBOL_KIND_ENUM_MEMBER,
            SymbolKind::ENUM => PhidiIcons::SYMBOL_KIND_ENUM,
            SymbolKind::EVENT => PhidiIcons::SYMBOL_KIND_EVENT,
            SymbolKind::FIELD => PhidiIcons::SYMBOL_KIND_FIELD,
            SymbolKind::FILE => PhidiIcons::SYMBOL_KIND_FILE,
            SymbolKind::INTERFACE => PhidiIcons::SYMBOL_KIND_INTERFACE,
            SymbolKind::KEY => PhidiIcons::SYMBOL_KIND_KEY,
            SymbolKind::FUNCTION => PhidiIcons::SYMBOL_KIND_FUNCTION,
            SymbolKind::METHOD => PhidiIcons::SYMBOL_KIND_METHOD,
            SymbolKind::OBJECT => PhidiIcons::SYMBOL_KIND_OBJECT,
            SymbolKind::NAMESPACE => PhidiIcons::SYMBOL_KIND_NAMESPACE,
            SymbolKind::NUMBER => PhidiIcons::SYMBOL_KIND_NUMBER,
            SymbolKind::OPERATOR => PhidiIcons::SYMBOL_KIND_OPERATOR,
            SymbolKind::TYPE_PARAMETER => PhidiIcons::SYMBOL_KIND_TYPE_PARAMETER,
            SymbolKind::PROPERTY => PhidiIcons::SYMBOL_KIND_PROPERTY,
            SymbolKind::STRING => PhidiIcons::SYMBOL_KIND_STRING,
            SymbolKind::STRUCT => PhidiIcons::SYMBOL_KIND_STRUCT,
            SymbolKind::VARIABLE => PhidiIcons::SYMBOL_KIND_VARIABLE,
            _ => return None,
        };
    ...
    fn terminal_get_named_color(
        ...
        let (color, alpha) = match color {
            alacritty_terminal::vte::ansi::NamedColor::Cursor => {
                (PhidiColor::TERMINAL_CURSOR, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Foreground => {
                (PhidiColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Background => {
                (PhidiColor::TERMINAL_BACKGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Blue => {
                (PhidiColor::TERMINAL_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Green => {
                (PhidiColor::TERMINAL_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Yellow => {
                (PhidiColor::TERMINAL_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Red => {
                (PhidiColor::TERMINAL_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::White => {
                (PhidiColor::TERMINAL_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Black => {
                (PhidiColor::TERMINAL_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Cyan => {
                (PhidiColor::TERMINAL_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Magenta => {
                (PhidiColor::TERMINAL_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlue => {
                (PhidiColor::TERMINAL_BRIGHT_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightGreen => {
                (PhidiColor::TERMINAL_BRIGHT_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightYellow => {
                (PhidiColor::TERMINAL_BRIGHT_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightRed => {
                (PhidiColor::TERMINAL_BRIGHT_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightWhite => {
                (PhidiColor::TERMINAL_BRIGHT_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlack => {
                (PhidiColor::TERMINAL_BRIGHT_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightCyan => {
                (PhidiColor::TERMINAL_BRIGHT_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightMagenta => {
                (PhidiColor::TERMINAL_BRIGHT_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightForeground => {
                (PhidiColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlack => {
                (PhidiColor::TERMINAL_BLACK, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimRed => {
                (PhidiColor::TERMINAL_RED, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimGreen => {
                (PhidiColor::TERMINAL_GREEN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimYellow => {
                (PhidiColor::TERMINAL_YELLOW, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlue => {
                (PhidiColor::TERMINAL_BLUE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimMagenta => {
                (PhidiColor::TERMINAL_MAGENTA, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimCyan => {
                (PhidiColor::TERMINAL_CYAN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimWhite => {
                (PhidiColor::TERMINAL_WHITE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimForeground => {
                (PhidiColor::TERMINAL_FOREGROUND, 0.66)
            }
        };
