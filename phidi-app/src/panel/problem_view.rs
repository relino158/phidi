...
fn file_view(
    ...
    let icon = match severity {
        DiagnosticSeverity::ERROR => PhidiIcons::ERROR,
        _ => PhidiIcons::WARNING,
    };
    let icon_color = move || {
        let config = config.get();
        match severity {
            DiagnosticSeverity::ERROR => config.color(PhidiColor::PHIDI_ERROR),
            _ => config.color(PhidiColor::PHIDI_WARN),
        }
    };
...
fn related_view(
    ...
    stack((
        ...
        stack((
            svg(move || config.get().ui_svg(PhidiIcons::LINK)).style(move |s| {
                let config = config.get();
                let size = config.ui.icon_size() as f32;
                s.size(size, size)
