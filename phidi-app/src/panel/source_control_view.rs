...
use phidi_core::buffer::rope_text::RopeText;
...
use crate::{
    ...
    config::{color::PhidiColor, icon::PhidiIcons},
                    ...
                    let view = stack((
                        ...
                        label(|| "Commit Message".to_string()).style(move |s| {
                            ...
                            s.absolute()
                                ...
                                .color(config.color(PhidiColor::EDITOR_DIM))
                        ...
                        s.margin_top(10.0)
                            ...
                            .active(|s| {
                                s.background(config.color(
                                    PhidiColor::PANEL_HOVERED_ACTIVE_BACKGROUND,
                                ))
                            })
                            .selectable(false)
                    })
...
fn file_diffs_view(source_control: SourceControlData) -> impl View {
    ...
    let phidi_command = source_control.common.phidi_command;
    ...
    let view_fn = move |(path, (diff, checked)): (PathBuf, (FileDiff, bool))| {
        ...
        stack((
            ...
            container({
                svg(move || {
                    let svg = match &diff {
                        FileDiff::Modified(_) => PhidiIcons::SCM_DIFF_MODIFIED,
                        FileDiff::Added(_) => PhidiIcons::SCM_DIFF_ADDED,
                        FileDiff::Deleted(_) => PhidiIcons::SCM_DIFF_REMOVED,
                        FileDiff::Renamed(_, _) => PhidiIcons::SCM_DIFF_RENAMED,
                    };
                ...
                .style(move |s| {
                    ...
                    let color = match &diff_for_style {
                        FileDiff::Modified(_) => PhidiColor::SOURCE_CONTROL_MODIFIED,
                        FileDiff::Added(_) => PhidiColor::SOURCE_CONTROL_ADDED,
                        FileDiff::Deleted(_) => PhidiColor::SOURCE_CONTROL_REMOVED,
                        FileDiff::Renamed(_, _) => {
                            PhidiColor::SOURCE_CONTROL_MODIFIED
                        }
                    };
            ...
            let discard = move || {
                phidi_command.send(PhidiCommand {
                    kind: CommandKind::Workbench(
                        PhidiWorkbenchCommand::SourceControlDiscardTargetFileChanges,
                    ),
        ...
        .style(move |s| {
            ...
            s.padding_left(10.0)
                ...
                .hover(|s| {
                    s.background(config.color(PhidiColor::PANEL_HOVERED_BACKGROUND))
                })
        })
    };
