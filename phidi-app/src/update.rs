...
pub fn get_latest_release() -> Result<ReleaseInfo> {
    let url = match meta::RELEASE {
        ...
        meta::ReleaseType::Nightly => {
            "https://api.github.com/repos/phidi/phidi/releases/tags/nightly"
        }
        _ => "https://api.github.com/repos/phidi/phidi/releases/latest",
    };

    let resp = phidi_proxy::get_url(url, Some("Phidi"))?;
...
pub fn download_release(release: &ReleaseInfo) -> Result<PathBuf> {
    ...
    let name = match std::env::consts::OS {
        "macos" => "Phidi-macos.dmg",
        "linux" => match std::env::consts::ARCH {
            "aarch64" => "phidi-linux-arm64.tar.gz",
            "x86_64" => "phidi-linux-amd64.tar.gz",
            _ => return Err(anyhow!("arch not supported")),
        },
        #[cfg(feature = "portable")]
        "windows" => "Phidi-windows-portable.zip",
        #[cfg(not(feature = "portable"))]
        "windows" => "Phidi-windows.msi",
    ...
    for asset in &release.assets {
        if asset.name == name {
            let mut resp = phidi_proxy::get_url(&asset.browser_download_url, None)?;
            if !resp.status().is_success() {
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::remove_dir_all(dest.join("Phidi.app"))?;
    fs_extra::copy_items(
        &[info.mount_point.join("Phidi.app")],
        dest,
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::copy(parent.join("Phidi").join("phidi"), process_path)?;
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::rename(process_path, dst_parent.join("phidi.exe.bak"))?;
    std::fs::copy(parent.join("phidi.exe"), process_path)?;
...
pub fn restart(path: &Path) -> Result<()> {
    ...
    let phidi_exe = std::env::current_exe()
        .map_err(|err| anyhow!("can't get path to exe").context(err))?;
    let phidi_exe = phidi_exe
        .to_str()
        .ok_or_else(|| anyhow!("can't convert exe path to str"))?;

    std::process::Command::new("cmd")
        .raw_arg(format!(
            r#"/C taskkill /PID {process_id} & msiexec /i "{path}" /qb & start "" "{phidi_exe}""#,
        ))
        .creation_flags(DETACHED_PROCESS)
        .spawn()?;
    Ok(())
...
pub fn cleanup() {
    ...
    if let Ok(process_path) = std::env::current_exe() {
        if let Some(dst_parent) = process_path.parent() {
            if let Err(err) = std::fs::remove_file(dst_parent.join("phidi.exe.bak"))
            {
                tracing::error!("{:?}", err);
            }
        }
