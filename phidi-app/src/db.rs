...
use crate::{
    ...
    workspace::{PhidiWorkspace, WorkspaceInfo},
};
...
impl PhidiDb {
    ...
    pub fn recent_workspaces(&self) -> Result<Vec<PhidiWorkspace>> {
        ...
        let workspaces: Vec<PhidiWorkspace> = serde_json::from_str(&workspaces)?;
        Ok(workspaces)
    }

    pub fn update_recent_workspace(&self, workspace: &PhidiWorkspace) -> Result<()> {
    ...
    fn insert_recent_workspace(&self, workspace: PhidiWorkspace) -> Result<()> {
...
fn workspace_folder_name(workspace: &PhidiWorkspace) -> String {
