...
use phidi_core::{buffer::rope_text::RopeText, mode::Mode};
...
use crate::{
    ...
    config::{
        DropdownInfo, PhidiConfig, color::PhidiColor, core::CoreConfig,
        editor::EditorConfig, icon::PhidiIcons, terminal::TerminalConfig,
        ui::UIConfig,
    },
...
impl KeyPressFocus for SettingsData {
    fn get_mode(&self) -> phidi_core::mode::Mode {
        Mode::Insert
    }
    ...
    fn run_command(
        &self,
        _command: &crate::command::PhidiCommand,
        _count: Option<usize>,
        _mods: Modifiers,
        ...
        .style(move |s| {
            ...
            s.padding_horiz(20.0)
                ...
                .apply_if(kind == current_kind.get(), |s| {
                    s.background(config.color(PhidiColor::PANEL_CURRENT_BACKGROUND))
                })
                .hover(|s| {
            ...
            if let Some(is_ticked) = is_ticked {
                ...
                create_effect(move |last| {
                    ...
                    if let Ok(value) = serde::Serialize::serialize(
                        ...
                        PhidiConfig::update_file(&kind, &field, value);
                    }
                });
                        ...
                        text("Reset")
                            ...
                            .style(move |s| {
                                ...
                                s.margin_left(10)
                                    ...
                                    .border_radius(6)
                                    .border_color(
                                        config.color(PhidiColor::PHIDI_BORDER),
                                    )
...
fn dropdown_view(
    ...
    stack((
        ...
        container(
            svg(move || {
                if expanded.get() {
                    config.get().ui_svg(PhidiIcons::CLOSE)
                } else {
                    config.get().ui_svg(PhidiIcons::DROPDOWN_ARROW)
                }
            })
            .style(move |s| {
                let config = config.get();
