...
use crate::{
    ...
    command::{
        CommandKind, InternalCommand, PhidiCommand, PhidiWorkbenchCommand,
        WindowCommand,
    },
    config::{
        PhidiConfig, color::PhidiColor, icon::PhidiIcons, ui::TabSeparatorHeight,
    ...
    workspace::{PhidiWorkspace, PhidiWorkspaceType},
};
...
#[derive(Debug)]
struct Cli {
    /// Launch new window even if Phidi is already running
    ...
    #[clap(value_parser = phidi_proxy::cli::parse_file_line_column)]
...
pub struct AppData {
    ...
    pub config: RwSignal<Arc<PhidiConfig>>,
...
impl AppData {
    pub fn reload_config(&self) {
        let config =
            PhidiConfig::load(&PhidiWorkspace::default(), &[], &self.plugin_paths);
    ...
    fn create_windows(
        ...
        if !dirs.is_empty() {
            ...
            for dir in dirs {
                ...
                {
                    PhidiWorkspaceType::RemoteWSL(crate::workspace::WslHost {
                        host: String::new(),
                    })
                } else {
                ...
                #[cfg(not(windows))]
                let workspace_type = PhidiWorkspaceType::Local;
        ...
        if inital_windows == 0 {
            let mut info = db.get_window().unwrap_or_else(|_| WindowInfo {
                ...
                tabs: TabsInfo {
                    active_tab: 0,
                    workspaces: vec![PhidiWorkspace::default()],
                },
            });
            info.tabs = TabsInfo {
                active_tab: 0,
    ...
    let view_fn = move |(i, layout_rect, child): (
                            ...
                            .apply_if(
                                !info.with(|info| info.is_pristine)
                                    && config.ui.tab_close_button
                                        == TabCloseButton::Off,
                                |s| s.color(config.color(PhidiColor::PHIDI_WARN)),
                            )
                    })
            })
            .style(|s| s.padding(4.));
            ...
            child_view
                ...
                .dragging_style(move |s| {
                    ...
                    s.border(1.0)
                        ...
                        .background(
                            config
                                .color(PhidiColor::PANEL_BACKGROUND)
                                .multiply_alpha(0.7),
                        )
                        .border_color(config.color(PhidiColor::PHIDI_BORDER))
                })
                ...
                .style(move |s| {
                    s.size_full()
                        ...
                        .border_color(config.get().color(if is_focused() {
                            PhidiColor::PHIDI_TAB_ACTIVE_UNDERLINE
                        } else {
                            PhidiColor::PHIDI_TAB_INACTIVE_UNDERLINE
                        }))
                })
                .style(|s| {
                    s.absolute()
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .border_color(
                            config
                                .get()
                                .color(PhidiColor::PHIDI_TAB_ACTIVE_UNDERLINE)
                                .multiply_alpha(0.5),
                        )
                })
        ...
        .style(move |s| {
            ...
            s.height_full()
                ...
                .hover(|s| s.background(config.color(PhidiColor::HOVER_BACKGROUND)))
        })
            ...
            (
                clip(empty().style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(config.color(PhidiColor::PANEL_BACKGROUND))
                        .box_shadow_blur(3.0)
                        .box_shadow_color(
                            config.color(PhidiColor::PHIDI_DROPDOWN_SHADOW),
                        )
                }))
                .style(move |s| {
                ...
                stack((
                    clickable_icon(
                        || PhidiIcons::TAB_PREVIOUS,
                        move || {
                            workbench_command
                                .send(PhidiWorkbenchCommand::PreviousEditorTab);
                        },
                        || false,
                        || false,
                        || "Previous Tab",
                    ...
                    clickable_icon(
                        ...
                        move || {
                            workbench_command
                                .send(PhidiWorkbenchCommand::NextEditorTab);
                        },
                        || false,
                        || false,
                        || "Next Tab",
                    ...
                    stack((
                        ...
                        .style(move |s| {
                            s.height_full()
                                ...
                                .border_right(1.0)
                                .border_color(
                                    config.get().color(PhidiColor::PHIDI_BORDER),
                                )
                        }),
                        container(
                            editor_container_view(
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(
                            config
                                .get()
                                .color(PhidiColor::EDITOR_DRAG_DROP_BACKGROUND),
                        )
                })
            ...
            .style(move |s| {
                ...
                s.position(Position::Absolute)
                    ...
                    .apply_if(is_dragging, |s| {
                        ...
                        .background(config.get().color(PhidiColor::EDITOR_CARET))
                    })
                    .hover(|s| {
                        s.cursor(match direction {
                            SplitDirection::Vertical => CursorStyle::ColResize,
    ...
    dyn_stack(
        ...
        move |(_, content)| {
            container(empty().style(move |s| {
                ...
                .background(config.get().color(PhidiColor::PHIDI_BORDER))
            }))
            .style(move |s| {
...
fn main_split(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.border_color(config.color(PhidiColor::PHIDI_BORDER))
            .background(config.color(PhidiColor::EDITOR_BACKGROUND))
    ...
    let view = container(
        svg(move || config.get().ui_svg(icon()))
            .style(move |s| {
                ...
                let size = config.ui.icon_size() as f32;
                s.size(size, size)
                    .color(config.color(PhidiColor::PHIDI_ICON_ACTIVE))
                    .disabled(|s| {
                        s.color(config.color(PhidiColor::PHIDI_ICON_INACTIVE))
    ...
    .style(move |s| {
        ...
        s.padding(4.0)
            ...
            .apply_if(active_fn(), |s| {
                s.border_color(config.color(PhidiColor::EDITOR_CARET))
            })
            .hover(|s| {
                s.cursor(CursorStyle::Pointer)
                    .background(config.color(PhidiColor::PANEL_HOVERED_BACKGROUND))
            })
            .active(|s| {
                s.background(
                    config.color(PhidiColor::PANEL_HOVERED_ACTIVE_BACKGROUND),
                )
            })
    });
...
fn tooltip_tip<V: View + 'static>(
    ...
    container(child).style(move |s| {
        ...
        s.padding_horiz(10.0)
            ...
            .color(config.color(PhidiColor::TOOLTIP_FOREGROUND))
            .background(config.color(PhidiColor::TOOLTIP_BACKGROUND))
            .border(1)
            .border_radius(6)
            .border_color(config.color(PhidiColor::PHIDI_BORDER))
            .box_shadow_blur(3.0)
            .box_shadow_color(config.color(PhidiColor::PHIDI_DROPDOWN_SHADOW))
            .margin_left(0.0)
            .margin_top(4.0)
    })
}
            ...
            container(
                stack((
                    ...
                    focus_text(
                        ...
                        move || config.get().color(PhidiColor::EDITOR_FOCUS),
                    )
            ...
            container(
                stack((
                    svg(move || {
                        ...
                        config
                            .symbol_svg(&kind)
                            .unwrap_or_else(|| config.ui_svg(PhidiIcons::FILE))
                    })
                    .style(move |s| {
                        let config = config.get();
                        let size = config.ui.icon_size() as f32;
                        s.min_width(size).size(size, size).margin_right(5.0).color(
                            config.symbol_color(&kind).unwrap_or_else(|| {
                                config.color(PhidiColor::PHIDI_ICON_ACTIVE)
                            }),
                        )
                    }),
                    focus_text(
            ...
            container(
                stack((
                    svg(move || {
                        let config = config.get();
                        match mode {
                            RunDebugMode::Run => config.ui_svg(PhidiIcons::START),
                            RunDebugMode::Debug => config.ui_svg(PhidiIcons::DEBUG),
                        }
                    })
                    .style(move |s| {
                        let config = config.get();
    ...
    .style(move |s| {
        s.width_full()
            ...
            .apply_if(index.get() == i, |style| {
                style.background(
                    config.get().color(PhidiColor::PALETTE_CURRENT_BACKGROUND),
                )
            })
    })
}
                    ...
                    .style(move |s| {
                        s.width_full().cursor(CursorStyle::Pointer).hover(|s| {
                            s.background(
                                config
                                    .get()
                                    .color(PhidiColor::PANEL_HOVERED_BACKGROUND),
                            )
                        })
                    })
                },
    ...
    container(
        ...
        .style(move |s| {
            ...
            s.width(config.ui.palette_width() as f64)
                ...
                .border_color(config.color(PhidiColor::PHIDI_BORDER))
                .flex_col()
                .background(config.color(PhidiColor::PALETTE_BACKGROUND))
                .pointer_events_auto()
        }),
    )
    .style(move |s| {
            ...
            stack((
                svg(move || {
                    if let MessageType::ERROR = message.typ {
                        config.get().ui_svg(PhidiIcons::ERROR)
                    } else {
                        config.get().ui_svg(PhidiIcons::WARNING)
                    }
                })
                .style(move |s| {
                    let config = config.get();
                    ...
                    let color = if let MessageType::ERROR = message.typ {
                        config.color(PhidiColor::PHIDI_ERROR)
                    } else {
                        config.color(PhidiColor::PHIDI_WARN)
                    };
                    s.min_width(size)
                        .size(size, size)
                        .margin_right(10.0)
...
fn completion(window_tab_data: Rc<WindowTabData>) -> impl View {
                    ...
                    .style(move |s| {
                        ...
                        s.padding_horiz(5.0)
                            ...
                            .apply_if(active.get() == i, |s| {
                                s.background(
                                    config.color(PhidiColor::COMPLETION_CURRENT),
                                )
                            })
                            .hover(move |s| {
                                s.background(
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.color(PhidiColor::COMPLETION_BACKGROUND))
...
fn code_action(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        .margin_left(origin.x as f32)
        .margin_top(origin.y as f32)
        .background(config.get().color(PhidiColor::COMPLETION_BACKGROUND))
        .border_radius(6.0)
    })
...
fn rename(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.get().color(PhidiColor::PANEL_BACKGROUND))
            .border_radius(6.0)
            .padding(6.0)
    })
...
fn window_tab(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        let config = config.get();
        s.size_full()
            .color(config.color(PhidiColor::EDITOR_FOREGROUND))
            .background(config.color(PhidiColor::EDITOR_BACKGROUND))
            .font_size(config.ui.font_size() as f32)
            ...
            .class(floem::views::scroll::Handle, |s| {
                s.background(config.color(PhidiColor::PHIDI_SCROLL_BAR))
            })
    })
...
fn workspace_title(workspace: &PhidiWorkspace) -> Option<String> {
    let p = workspace.path.as_ref()?;
    let dir = p.file_name().unwrap_or(p.as_os_str()).to_string_lossy();
    Some(match &workspace.kind {
        PhidiWorkspaceType::Local => format!("{dir}"),
        PhidiWorkspaceType::RemoteSSH(remote) => format!("{dir} [{remote}]"),
        #[cfg(windows)]
        PhidiWorkspaceType::RemoteWSL(remote) => format!("{dir} [{remote}]"),
    })
}
        ...
        stack((
            ...
            .dragging_style(move |s| {
                ...
                s.border(1.0)
                    ...
                    .color(
                        config
                            .color(PhidiColor::EDITOR_FOREGROUND)
                            .multiply_alpha(0.7),
                    )
                    .background(
                        config
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    .height_full()
                    .border_color(
                        config.get().color(PhidiColor::PHIDI_TAB_ACTIVE_UNDERLINE),
                    )
        ...
        container(clickable_icon(
            ...
            move || {
                window_data.run_window_command(WindowCommand::NewWorkspaceTab {
                    workspace: PhidiWorkspace::default(),
                    end: true,
                });
            },
            || false,
...
fn window(window_data: WindowData) -> impl View {
    ...
    .window_title(move || {
        ...
        match workspace {
            Some(workspace) => format!("{workspace} - Phidi"),
            None => "Phidi".to_string(),
        }
    })
    ...
    .window_menu(move || {
        ...
        if let Some((_, window_tab)) = window_tab {
            ...
            let phidi_command = window_tab.common.phidi_command;
            window_menu(phidi_command, workbench_command)
        } else {
            Menu::new("Phidi")
        }
    })
...
pub fn launch() {
    ...
    trace!(TraceLevel::INFO, "Starting up Phidi..");
    ...
    if !cli.wait {
        ...
        if let Err(why) = cmd
            ...
            .stderr(stderr)
            .stdout(stdout)
            .env("PHIDI_LOG", "phidi_app::app=error,off")
            .spawn()
        {
            eprintln!("Failed to launch phidi: {why}");
            std::process::exit(1);
        };
        return;
    }
    ...
    if let Err(err) = phidi_proxy::register_phidi_path() {
        tracing::error!("{:?}", err);
    }
    let db = match PhidiDb::new() {
        Ok(db) => Arc::new(db),
        Err(e) => {
            #[cfg(windows)]
            logging::error_modal("Error", &format!("Failed to create PhidiDb: {e}"));

            trace!(TraceLevel::ERROR, "Failed to create PhidiDb: {e}");
            std::process::exit(1);
        }
    };
    let scope = Scope::new();
    ...
    if let Some(path) = PhidiConfig::settings_file() {
    ...
    if let Some(path) = PhidiConfig::keymaps_file() {
        if let Err(err) = watcher.watch(&path, notify::RecursiveMode::Recursive) {
            tracing::error!("{:?}", err);
        }
    }
    ...
    let config = PhidiConfig::load(&PhidiWorkspace::default(), &[], &plugin_paths);
...
pub fn try_open_in_existing_process(
    ...
    phidi_rpc::stdio::write_msg(&mut socket, msg)?;
        ...
        std::thread::spawn(move || -> Result<()> {
            ...
            loop {
                let msg: Option<CoreMessage> =
                    phidi_rpc::stdio::read_msg(&mut reader)?;
...
pub fn window_menu(
    phidi_command: Listener<PhidiCommand>,
    workbench_command: Listener<PhidiWorkbenchCommand>,
) -> Menu {
    Menu::new("Phidi")
        .entry({
            let mut menu = Menu::new("Phidi")
                .entry(MenuItem::new("About Phidi").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::ShowAbout)
                }))
                .separator()
                .entry(
                    Menu::new("Settings...")
                        .entry(MenuItem::new("Open Settings").action(move || {
                            workbench_command
                                .send(PhidiWorkbenchCommand::OpenSettings);
                        }))
                        .entry(MenuItem::new("Open Keyboard Shortcuts").action(
                            move || {
                                workbench_command.send(
                                    PhidiWorkbenchCommand::OpenKeyboardShortcuts,
                                );
                            },
                        )),
                )
                .separator()
                .entry(MenuItem::new("Quit Phidi").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::Quit);
                }));
            if cfg!(target_os = "macos") {
                menu = menu
                    .separator()
                    .entry(MenuItem::new("Hide Phidi"))
        ...
        .entry(
            Menu::new("File")
                .entry(MenuItem::new("New File").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::NewFile);
                }))
                .separator()
                .entry(MenuItem::new("Open").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::OpenFile);
                }))
                .entry(MenuItem::new("Open Folder").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::OpenFolder);
                }))
                .separator()
                .entry(MenuItem::new("Save").action(move || {
                    phidi_command.send(PhidiCommand {
                        kind: CommandKind::Focus(FocusCommand::Save),
                        data: None,
                    });
                }))
                .entry(MenuItem::new("Save All").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::SaveAll);
                }))
                .separator()
                .entry(MenuItem::new("Close Folder").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::CloseFolder);
                }))
                .entry(MenuItem::new("Close Window").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::CloseWindow);
                })),
        )
        .entry(
            Menu::new("Edit")
