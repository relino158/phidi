...
use nucleo::Utf32Str;
use phidi_core::{
    buffer::rope_text::RopeText, command::FocusCommand, language::PhidiLanguage,
    line_ending::LineEnding, mode::Mode, movement::Movement, selection::Selection,
    syntax::Syntax,
};
use phidi_rpc::proxy::ProxyResponse;
...
use crate::{
    command::{
        CommandExecuted, CommandKind, InternalCommand, PhidiCommand, WindowCommand,
    },
    db::PhidiDb,
    ...
    workspace::{PhidiWorkspace, PhidiWorkspaceType, SshHost},
};
...
impl PaletteData {
        ...
        let items = workspaces
            ...
            .filter_map(|w| {
                ...
                let filter_text = match &w.kind {
                    PhidiWorkspaceType::Local => text,
                    PhidiWorkspaceType::RemoteSSH(remote) => {
                        format!("[{remote}] {text}")
                    }
                    #[cfg(windows)]
                    PhidiWorkspaceType::RemoteWSL(remote) => {
                        format!("[{remote}] {text}")
                    }
                };
                Some(PaletteItem {
    ...
    fn get_ssh_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let PhidiWorkspaceType::RemoteSSH(host) = &workspace.kind {
    ...
    fn get_wsl_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let PhidiWorkspaceType::RemoteWSL(host) = &workspace.kind {
    ...
    fn set_run_configs(&self, content: String) {
        ...
        if configs.is_none() {
            if let Some(path) = self.workspace.path.as_ref() {
                let path = path.join(".phidi").join("run.toml");
                self.common
                    .internal_command
    ...
    fn get_run_configs(&self) {
        if let Some(workspace) = self.common.workspace.path.as_deref() {
            let run_toml = workspace.join(".phidi").join("run.toml");
    ...
    fn get_languages(&self) {
        let langs = PhidiLanguage::languages();
        let items = langs
            .iter()
        ...
        if let Some(item) = items.get(index) {
            match &item.content {
                PaletteItemContent::PaletteHelp { cmd } => {
                    let cmd = PhidiCommand {
                        kind: CommandKind::Workbench(cmd.clone()),
                        data: None,
                    };

                    self.common.phidi_command.send(cmd);
                }
                ...
                PaletteItemContent::Command { cmd } => {
                    self.common.phidi_command.send(cmd.clone());
                }
                    ...
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: PhidiWorkspace {
                                kind: PhidiWorkspaceType::RemoteSSH(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::WslHost { host } => {
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: PhidiWorkspace {
                                kind: PhidiWorkspaceType::RemoteWSL(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::Language { name } => {
                    ...
                    if name.is_empty() || name.to_lowercase().eq("plain text") {
                        ...
                        let lang = match PhidiLanguage::from_name(name) {
                            Some(v) => v,
                            None => return,
                        };
                ...
                PaletteItemContent::SCMReference { name } => {
                    self.common
                        .phidi_command
                        .send(crate::command::PhidiCommand {
                        kind: CommandKind::Workbench(
                            crate::command::PhidiWorkbenchCommand::CheckoutReference,
                        ),
            ...
            self.common.window_common.window_command.send(
                WindowCommand::SetWorkspace {
                    workspace: PhidiWorkspace {
                        kind: PhidiWorkspaceType::RemoteSSH(ssh),
                        path: None,
                        last_open: 0,
                    },
                },
