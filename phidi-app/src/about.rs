...
use phidi_core::{command::FocusCommand, meta::VERSION, mode::Mode};

use crate::{
    command::{CommandExecuted, CommandKind},
    config::color::PhidiColor,
...
impl AboutUri {
    const PHIDI: &'static str = "https://phidi.dev";
    const GITHUB: &'static str = "https://github.com/phidi/phidi";
    const MATRIX: &'static str = "https://matrix.to/#/#phidi-editor:matrix.org";
        ...
        stack((
            svg(move || (config.get()).logo_svg()).style(move |s| {
                s.size(logo_size, logo_size)
                    .color(config.get().color(PhidiColor::EDITOR_FOREGROUND))
            }),
            label(|| "Phidi".to_string()).style(move |s| {
                s.font_bold()
                    .margin_top(10.0)
                    .color(config.get().color(PhidiColor::EDITOR_FOREGROUND))
            }),
            ...
            web_link(
                || "Website".to_string(),
                || AboutUri::PHIDI.to_string(),
                move || config.get().color(PhidiColor::EDITOR_LINK),
                internal_command,
            )
...
fn exclusive_popup<V: View + 'static>(
    ...
    .style(move |s| {
        ...
        .background(
            config
                .get()
                .color(PhidiColor::PHIDI_DROPDOWN_SHADOW)
                .multiply_alpha(0.5),
        )
    })
}
