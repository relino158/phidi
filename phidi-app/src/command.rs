...
pub enum CommandKind {
    Workbench(PhidiWorkbenchCommand),
    Edit(EditCommand),
    Move(MoveCommand),
...
pub fn phidi_internal_commands() -> IndexMap<String, PhidiCommand> {
    let mut commands = IndexMap::new();

    for c in PhidiWorkbenchCommand::iter() {
        let command = PhidiCommand {
...
pub enum PhidiWorkbenchCommand {
    ...
    #[cfg(target_os = "macos")]
    #[strum(message = "Install Phidi to PATH")]
    #[strum(serialize = "install_to_path")]
    InstallToPATH,

    #[cfg(target_os = "macos")]
    #[strum(message = "Uninstall Phidi from PATH")]
