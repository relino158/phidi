...
use phidi_rpc::{plugin::PluginId, proxy::ProxyRpcHandler};

use crate::{config::PhidiConfig, editor::EditorData, snippet::Snippet};
...
impl CompletionData {
    pub fn new(cx: Scope, config: ReadSignal<Arc<PhidiConfig>>) -> Self {
...
fn completion_lens_text(
    ...
    // Get only the first line of text, because Phidi does not currently support
