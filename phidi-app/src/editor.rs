...
use phidi_rpc::{buffer::BufferId, plugin::PluginId, proxy::ProxyResponse};
use phidi_xi_rope::{Rope, RopeDelta, Transformer};
...
use crate::{
    command::{CommandKind, InternalCommand, PhidiCommand, PhidiWorkbenchCommand},
...
impl EditorData {
    ...
    fn run_edit_command(&self, cmd: &EditCommand) -> CommandExecuted {
        ...
        let yank_data =
            if let phidi_core::cursor::CursorMode::Visual { .. } = &cursor.mode {
    ...
    fn run_multi_selection_command(
        ...
        // This is currently special-cased in Phidi because floem editor does not have 'find'
        match cmd {
    ...
    fn run_move_command(
        &self,
        movement: &phidi_core::movement::Movement,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
            ...
            FocusCommand::JumpToNextSnippetPlaceholder => {
                self.snippet.update(|snippet| {
                    if let Some(snippet_mut) = snippet.as_mut() {
                        ...
                        {
                            let mut selection =
                                phidi_core::selection::Selection::new();
                            let region = phidi_core::selection::SelRegion::new(
                                *start, *end, None,
                            );
    ...
    fn inline_find(&self, direction: InlineFindDirection, c: &str) {
        ...
        if let Some(new_index) = match direction {
            ...
            self.run_move_command(
                &phidi_core::movement::Movement::Offset(
    ...
    fn on_screen_find(&self, pattern: &str) -> Vec<SelRegion> {
        ...
        if let Some((_, offset, _)) = items.first().copied() {
            self.run_move_command(
                &phidi_core::movement::Movement::Offset(offset),
                None,
                Modifiers::empty(),
            );
        }
    ...
    fn apply_completion_item(&self, item: &CompletionItem) -> anyhow::Result<()> {
        ...
        let additional_edit: Vec<_> = item
            ...
            .flatten()
            .map(|edit| {
                let selection = phidi_core::selection::Selection::region(
                    buffer.offset_of_position(&edit.range.start),
                    buffer.offset_of_position(&edit.range.end),
                );
                (selection, edit.new_text.as_str())
            })
            .collect::<Vec<(phidi_core::selection::Selection, &str)>>();
    ...
    pub fn completion_apply_snippet(
        ...
        let mut selection = phidi_core::selection::Selection::new();
        let (_tab, (start, end)) = &snippet_tabs[0];
        let region = phidi_core::selection::SelRegion::new(*start, *end, None);
        selection.add_region(region);
        cursor.set_insert(selection);
    ...
    fn search_whole_word_forward(&self, mods: Modifiers) {
        ...
        if let Some((start, _end)) = next {
            self.run_move_command(
                &phidi_core::movement::Movement::Offset(start),
                None,
                mods,
            );
        }
    ...
    pub fn word_at_cursor(&self) -> String {
        ...
        let region = self.cursor().with_untracked(|c| match &c.mode {
            phidi_core::cursor::CursorMode::Normal(offset) => {
                phidi_core::selection::SelRegion::caret(*offset)
            }
            phidi_core::cursor::CursorMode::Visual {
                start,
                end,
                mode: _,
            } => phidi_core::selection::SelRegion::new(
                *start.min(end),
                doc.buffer.with_untracked(|buffer| {
            ...
            phidi_core::cursor::CursorMode::Insert(selection) => {
                        ...
                        && pointer_event.modifiers.control())
                {
                    ...
                    match rs {
                        ...
                        | FindHintRs::NoMatchContinue { .. } => {
                            self.common.phidi_command.send(PhidiCommand {
    ...
    fn right_click(&self, pointer_event: &PointerInputEvent) {
                ...
                .unwrap_or_default()
            {
                vec![
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::RevealInPanel,
                    )),
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::RevealInFileExplorer,
                    )),
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::SourceControlOpenActiveFileRemoteUrl,
                    )),
                    None,
                    ...
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::AddRunDebugConfig,
                    )),
                    None,
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::PaletteCommand,
                    )),
                ]
            } else {
                vec![
                    ...
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::ShowCallHierarchy,
                    )),
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::FindReferences,
                    )),
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::GoToImplementation,
                    )),
                    Some(CommandKind::Focus(FocusCommand::Rename)),
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::RunInTerminal,
                    )),
                    None,
                    Some(CommandKind::Workbench(
                        PhidiWorkbenchCommand::RevealInPanel,
        ...
        if self.diff_editor_id.get_untracked().is_some() && is_file {
            cmds.push(Some(CommandKind::Workbench(
                PhidiWorkbenchCommand::GoToLocation,
            )));
        }
        let phidi_command = self.common.phidi_command;
        for cmd in cmds {
            if let Some(cmd) = cmd {
                menu = menu.entry(
...
impl KeyPressFocus for EditorData {
    ...
    fn run_command(
        &self,
        command: &crate::command::PhidiCommand,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
        if self.common.find.visual.get_untracked() && self.find_focus.get_untracked()
    ...
    match cmd {
        ...
        | EditCommand::DeleteForwardAndInsert => {
            let start = match deltas.first().and_then(|delta| delta.1.els.first()) {
                Some(phidi_xi_rope::DeltaElement::Copy(_, start)) => *start,
                _ => 0,
            };

            let end = match deltas.first().and_then(|delta| delta.1.els.get(1)) {
                Some(phidi_xi_rope::DeltaElement::Copy(end, _)) => *end,
                _ => 0,
            };
