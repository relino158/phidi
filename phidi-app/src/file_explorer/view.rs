...
use phidi_core::selection::Selection;
use phidi_rpc::{
...
fn file_node_text_color(
    ...
    let color = match diff {
        ...
        Some(FileDiffKind::Added) => PhidiColor::SOURCE_CONTROL_ADDED,
        Some(FileDiffKind::Deleted) => PhidiColor::SOURCE_CONTROL_REMOVED,
        None => PhidiColor::PANEL_FOREGROUND,
    };
                        ...
                        move |s| {
                            s.height(ui_line_height.get())
                                .color(
                                    config
                                        .get()
                                        .color(PhidiColor::PANEL_FOREGROUND_DIM),
                                )
                                .selectable(false)
                        },
                    ),
            ...
            stack((
                ...
                label(move || err.clone()).style(move |s| {
                    ...
                    let editor_background_color =
                        config.color(PhidiColor::PANEL_CURRENT_BACKGROUND);
                    let error_background_color =
                        config.color(PhidiColor::ERROR_LENS_ERROR_BACKGROUND);
                    ...
                    s.position(Position::Absolute)
                        ...
                        .width_full()
                        .color(config.color(PhidiColor::ERROR_LENS_ERROR_FOREGROUND))
                ...
                let view = stack((
                    svg(move || {
                        ...
                        let svg_str = match open {
                            true => PhidiIcons::ITEM_OPENED,
                            false => PhidiIcons::ITEM_CLOSED,
                        };
                    ...
                    }),
                    {
                        ...
                        svg(move || {
                            ...
                            if is_dir {
                                let svg_str = match open {
                                    true => PhidiIcons::DIRECTORY_OPENED,
                                    false => PhidiIcons::DIRECTORY_CLOSED,
                                };
                                config.ui_svg(svg_str)
                            } else if let Some(path) = kind.path() {
                                config.file_svg(path).0
                            } else {
                                config.ui_svg(PhidiIcons::FILE)
                            }
                        })
                        .style(move |s| {
                            let config = config.get();
                        ...
                        s.padding_right(15.0)
                            ...
                            .apply_if(
                                ...
                                |x| {
                                    x.background(
                                        config.get().color(
                                            PhidiColor::PANEL_CURRENT_BACKGROUND,
                                        ),
                                    )
                                },
                            )
