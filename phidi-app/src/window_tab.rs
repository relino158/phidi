...
use crate::{
    ...
    command::{
        CommandExecuted, CommandKind, InternalCommand, PhidiCommand,
        PhidiWorkbenchCommand, WindowCommand,
    },
    completion::{CompletionData, CompletionStatus},
    config::PhidiConfig,
    db::PhidiDb,
    debug::{DapData, PhidiBreakpoint, RunDebugMode, RunDebugProcess},
    doc::DocContent,
    ...
    workspace::{PhidiWorkspace, PhidiWorkspaceType, WorkspaceInfo},
};
...
pub struct CommonData {
    pub workspace: Arc<PhidiWorkspace>,
    pub scope: Scope,
    ...
    pub phidi_command: Listener<PhidiCommand>,
    pub workbench_command: Listener<PhidiWorkbenchCommand>,
...
pub struct WindowTabData {
    ...
    pub proxy: ProxyData,
    pub set_config: WriteSignal<Arc<PhidiConfig>>,
...
impl WindowTabData {
    ...
    pub fn new(
        ...
        let config = PhidiConfig::load(
            &workspace,
            &all_disabled_volts,
            &window_common.extra_plugin_paths,
        );
        let phidi_command = Listener::new_empty(cx);
                    ...
                    .map(|(path, breakpoints)| {
                        (
                            ...
                            breakpoints
                                .into_iter()
                                .map(|b| (b.line, b))
                                .collect::<BTreeMap<usize, PhidiBreakpoint>>(),
                        )
                    })
                    .collect(),
            );
...

        {
            ...
            window_tab_data.common.phidi_command.listen(move |cmd| {
                window_tab_data.run_phidi_command(cmd);
            });
        }
    ...
    pub fn run_phidi_command(&self, cmd: PhidiCommand) {
        match cmd.kind {
        ...
        match cmd {
            ...
            InstallToPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        ...
                        arguments: vec![String::from("-e"), format!(r#"do shell script "ln -sf '{}' /usr/local/bin/phidi" with administrator privileges"#, std::env::args().next().unwrap())],
                    }
                )
            }
            #[cfg(target_os = "macos")]
            UninstallFromPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        program: String::from("osascript"),
                        arguments: vec![String::from("-e"), String::from(r#"do shell script "rm /usr/local/bin/phidi" with administrator privileges"#)],
                    }
                )
            }
            JumpLocationForward => {
