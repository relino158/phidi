...
pub fn start_remote(
    ...
    let remote_proxy_path = match platform {
        Windows => format!(
            "%HOMEDRIVE%%HOMEPATH%\\AppData\\Local\\phidi\\{}\\data\\proxy",
            meta::NAME
        ),
        Darwin => format!(
            "~/Library/Application\\ Support/dev.phidi.{}/proxy",
            meta::NAME
        ),
        _ => {
    ...
    let remote_proxy_file = match platform {
        Windows => format!("{remote_proxy_path}\\phidi.exe"),
        _ => format!("{remote_proxy_path}/phidi"),
    };
    ...
    if !remote
        ...
        .map(|output| {
            if meta::RELEASE == ReleaseType::Debug {
                String::from_utf8_lossy(&output.stdout).starts_with("Phidi-proxy")
            } else {
                String::from_utf8_lossy(&output.stdout).trim()
                    == format!("Phidi-proxy {}", meta::VERSION)
            }
        })
        .unwrap_or(false)
    {
...
fn download_remote(
    ...
    let script_install = match platform {
        HostPlatform::Windows => {
            ...
            let remote_proxy_script = "${env:TEMP}\\phidi-proxy.ps1";
    ...
    if !script_install.success() {
        ...
        if !cmd.success() {
            let proxy_filename = format!("phidi-proxy-{platform}-{architecture}");
            ...
            let url = format!(
                "https://github.com/phidi/phidi/releases/download/{proxy_version}/{proxy_filename}.gz"
            );
            debug!("proxy download URI: {url}");
            let mut resp = phidi_proxy::get_url(url, None).expect("request failed");
