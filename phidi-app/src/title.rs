...
use phidi_core::meta;
use phidi_rpc::proxy::ProxyStatus;

use crate::{
    app::{clickable_icon, not_clickable_icon, tooltip_label, window_menu},
    command::{PhidiCommand, PhidiWorkbenchCommand, WindowCommand},
    config::{PhidiConfig, color::PhidiColor, icon::PhidiIcons},
    listener::Listener,
...
fn left(
    ...
    stack((
        ...
        container(svg(move || config.get().ui_svg(PhidiIcons::LOGO)).style(
            move |s| {
        ...
        .popout_menu(move || window_menu(phidi_command, workbench_command))
        .style(move |s| {
            s.margin_left(4.0)
                .margin_right(6.0)
        ...
        tooltip_label(
            config,
            container(svg(move || config.get().ui_svg(PhidiIcons::REMOTE)).style(
                move |s| {
                    let config = config.get();
                    ...
                    s.size(size, size).color(if is_local {
                        ...
                        match proxy_status.get() {
                            Some(_) => Color::WHITE,
                            None => config.color(PhidiColor::PHIDI_ICON_ACTIVE),
                        }
                    })
                },
            )),
        ...
        .popout_menu(move || {
            ...
            let mut menu = Menu::new("").entry(
                MenuItem::new("Connect to SSH Host").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::ConnectSshHost);
                }),
            );
            if !is_local
                ...
                })
            {
                menu = menu.entry(MenuItem::new("Disconnect remote").action(
                    move || {
                        workbench_command
                            .send(PhidiWorkbenchCommand::DisconnectRemote);
                    },
                ));
            }
            #[cfg(windows)]
            {
                menu = menu.entry(MenuItem::new("Connect to WSL Host").action(
                    move || {
                        workbench_command
                            .send(PhidiWorkbenchCommand::ConnectWslHost);
                    },
                ));
            }
            menu
            ...
            let color = if is_local {
                ...
                match proxy_status.get() {
                    Some(ProxyStatus::Connected) => {
                        config.color(PhidiColor::PHIDI_REMOTE_CONNECTED)
                    }
                    Some(ProxyStatus::Connecting) => {
                        config.color(PhidiColor::PHIDI_REMOTE_CONNECTING)
                    }
                    Some(ProxyStatus::Disconnected) => {
                        config.color(PhidiColor::PHIDI_REMOTE_DISCONNECTED)
                    }
...
fn middle(
    ...
    let jump_backward = move || {
        clickable_icon(
            || PhidiIcons::LOCATION_BACKWARD,
            move || {
                workbench_command.send(PhidiWorkbenchCommand::JumpLocationBackward);
            },
            || false,
    ...
    let jump_forward = move || {
        clickable_icon(
            || PhidiIcons::LOCATION_FORWARD,
            move || {
                workbench_command.send(PhidiWorkbenchCommand::JumpLocationForward);
            },
            || false,
        ...
        .popout_menu(move || {
            Menu::new("")
                ...
                .entry(MenuItem::new("Open Recent Workspace").action(move || {
                    workbench_command.send(PhidiWorkbenchCommand::PaletteWorkspace);
                }))
        })
    };
    ...
    stack((
        ...
        container(
            stack((
                svg(move || config.get().ui_svg(PhidiIcons::SEARCH)).style(
                    move |s| {
                        let config = config.get();
        ...
        .on_click_stop(move |_| {
            if workspace.clone().path.is_some() {
                workbench_command.send(PhidiWorkbenchCommand::PaletteHelpAndFile);
            } else {
                workbench_command.send(PhidiWorkbenchCommand::PaletteWorkspace);
            }
        })
        ...
        stack((
            clickable_icon(
                || PhidiIcons::START,
                move || {
                    workbench_command.send(PhidiWorkbenchCommand::PaletteRunAndDebug)
                },
                || false,
                || false,
                || "Run and Debug",
    ...
    stack((
        ...
        stack((
            ...
            .popout_menu(move || {
                Menu::new("")
                    .entry(MenuItem::new("Command Palette").action(move || {
                        workbench_command.send(PhidiWorkbenchCommand::PaletteCommand)
                    }))
                    .separator()
                    .entry(MenuItem::new("Open Settings").action(move || {
                        workbench_command.send(PhidiWorkbenchCommand::OpenSettings)
                    }))
                    .entry(MenuItem::new("Open Keyboard Shortcuts").action(
                        move || {
                            workbench_command
                                .send(PhidiWorkbenchCommand::OpenKeyboardShortcuts)
                        },
                    ))
                    .entry(MenuItem::new("Open Theme Color Settings").action(
                        move || {
                            workbench_command
                                .send(PhidiWorkbenchCommand::OpenThemeColorSettings)
                        },
                    ))
                    .separator()
                    .entry(if let Some(v) = latest_version.get_untracked() {
                        if update_in_progress.get_untracked() {
                            ...
                            MenuItem::new(format!("Restart to update ({v})")).action(
                                move || {
                                    workbench_command
                                        .send(PhidiWorkbenchCommand::RestartToUpdate)
                                },
                            )
                        }
                    } else {
            ...
            container(label(|| "1".to_string()).style(move |s| {
                let config = config.get();
                s.font_size(10.0)
                    .color(config.color(PhidiColor::EDITOR_BACKGROUND))
                    .border_radius(100.0)
                    .margin_left(5.0)
                    .margin_top(10.0)
                    .background(config.color(PhidiColor::EDITOR_CARET))
            }))
            .style(move |s| {
...
pub fn title(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    let phidi_command = window_tab_data.common.phidi_command;
        ...
        clickable_icon(
            move || {
                if window_maximized.get() {
                    PhidiIcons::WINDOW_RESTORE
                } else {
                    PhidiIcons::WINDOW_MAXIMIZE
                }
            },
            move || {
