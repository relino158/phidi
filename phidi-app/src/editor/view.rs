...
use phidi_xi_rope::find::CaseMatching;

use super::{DocSignal, EditorData, gutter::editor_gutter_view};
use crate::{
    app::clickable_icon,
    command::InternalCommand,
    config::{PhidiConfig, color::PhidiColor, editor::WrapStyle, icon::PhidiIcons},
    debug::{DapData, PhidiBreakpoint},
    doc::DocContent,
...
fn editor_wrap(config: &PhidiConfig) -> WrapMethod {
...
pub fn editor_style(
    ...
    .set(CursorColor, config.color(PhidiColor::EDITOR_CARET))
    .set(SelectionColor, config.color(PhidiColor::EDITOR_SELECTION))
    .set(
        CurrentLineColor,
        config.color(PhidiColor::EDITOR_CURRENT_LINE),
    )
    .set(
        VisibleWhitespaceColor,
        config.color(PhidiColor::EDITOR_VISIBLE_WHITESPACE),
    )
    .set(
        IndentGuideColor,
        config.color(PhidiColor::EDITOR_INDENT_GUIDE),
    )
    .set(ScrollBeyondLastLine, config.editor.scroll_beyond_last_line)
    .color(config.color(PhidiColor::EDITOR_FOREGROUND))
    .set(TextColor, config.color(PhidiColor::EDITOR_FOREGROUND))
    .set(PhantomColor, config.color(PhidiColor::EDITOR_DIM))
    .set(PlaceholderColor, config.color(PhidiColor::EDITOR_DIM))
    .set(
        PreeditUnderlineColor,
        config.color(PhidiColor::EDITOR_FOREGROUND),
    )
...
impl EditorView {
            ...
            match section.kind {
                ...
                DiffSectionKind::Added => {
                    cx.fill(
                        ...
                        config
                            .color(PhidiColor::SOURCE_CONTROL_ADDED)
                            .multiply_alpha(0.2),
                        0.0,
                    );
                }
                DiffSectionKind::Removed => {
                    cx.fill(
                        ...
                        config
                            .color(PhidiColor::SOURCE_CONTROL_REMOVED)
                            .multiply_alpha(0.2),
                        0.0,
                    );
                }
    ...
    fn paint_diff_no_code(
            ...
            .step_by(8)
        {
            ...
            if height > 0.0 {
                ...
                cx.stroke(
                    &Line::new(p0, p1),
                    config.color(PhidiColor::EDITOR_DIM),
                    &Stroke::new(1.0),
                );
            }
        }
    ...
    fn paint_current_line(
        ...
        if let Some(breakline) = breakline {
            if let Some(info) = screen_lines.info_for_line(breakline) {
                ...
                cx.fill(
                    &rect,
                    config.color(PhidiColor::EDITOR_DEBUG_BREAK_LINE),
                    0.0,
                );
            }
        }
    ...
    fn paint_find(&self, cx: &mut PaintCx, screen_lines: &ScreenLines) {
        ...
        let config = config.get_untracked();
        let line_height = config.editor.line_height() as f64;
        let color = config.color(PhidiColor::EDITOR_FOREGROUND);
    ...
    fn paint_sticky_headers(
        ...
        cx.fill(
            &sticky_area_rect,
            config.color(PhidiColor::EDITOR_STICKY_HEADER_BACKGROUND),
            0.0,
        );
    ...
    fn paint_scroll_bar(
        ...
        cx.fill(
            &Rect::ZERO
                ...
                .inflate(0.0, 10.0),
            config.color(PhidiColor::PHIDI_SCROLL_BAR),
            0.0,
        );
    ...
    fn paint_scope_lines(
        ...
        let config = self.editor.common.config.get_untracked();
        let line_height = config.editor.line_height() as f64;
        let brush = config.color(PhidiColor::EDITOR_FOREGROUND);
        ...
        stack((
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    // .border_bottom(1.0)
                    // .border_color(
                    //     config.get_color(PhidiColor::PHIDI_BORDER),
                    // )
                    .apply_if(
...
fn editor_gutter_breakpoint_view(
    ...
    breakpoints: RwSignal<BTreeMap<PathBuf, BTreeMap<usize, PhidiBreakpoint>>>,
    ...
    container(
        svg(move || config.get().ui_svg(PhidiIcons::DEBUG_BREAKPOINT)).style(
            move |s| {
                let config = config.get();
                let size = config.ui.icon_size() as f32 + 2.0;
                s.size(size, size)
                    .color(config.color(PhidiColor::DEBUG_BREAKPOINT_HOVER))
...
fn editor_gutter_breakpoints(
        ...
        stack((
            ...
            dyn_stack(
                ...
                move |(line, breakpoint)| {
                    ...
                    container(
                        svg(move || {
                            config.get().ui_svg(PhidiIcons::DEBUG_BREAKPOINT)
                        })
                        .style(move |s| {
                            let config = config.get();
                            let size = config.ui.icon_size() as f32 + 2.0;
                            let color = if active {
                                PhidiColor::DEBUG_BREAKPOINT
                            } else {
                                PhidiColor::EDITOR_DIM
                            };
    ...
    .style(move |s| {
        s.absolute()
            ...
            .background(config.get().color(PhidiColor::EDITOR_BACKGROUND))
    })
}
...
fn editor_gutter_code_lens_view(
    ...
    let view = container(svg(move || config.get().ui_svg(PhidiIcons::START)).style(
        move |s| {
            let config = config.get();
    ...
    let view = container(
        svg(move || {
            let icon_str = match folding_display_item {
                FoldingDisplayItem::UnfoldStart(_) => PhidiIcons::FOLD_DOWN,
                FoldingDisplayItem::Folded(_) => PhidiIcons::FOLD,
                FoldingDisplayItem::UnfoldEnd(_) => PhidiIcons::FOLD_UP,
            };
...
fn editor_gutter_code_actions(
    ...
    container(
        container(
            svg(move || config.get().ui_svg(PhidiIcons::LIGHTBULB)).style(
                move |s| {
                    let config = config.get();
                    let size = config.ui.icon_size() as f32;
                    s.size(size, size)
            ...
            s.padding(4.0)
                ...
                .hover(|s| {
                    s.cursor(CursorStyle::Pointer).background(
                        config.color(PhidiColor::PANEL_HOVERED_BACKGROUND),
                    )
                })
                .active(|s| {
                    s.background(
                        ...
                        move |(i, section)| {
                            stack((
                                svg(move || {
                                    config
                                        .get()
                                        .ui_svg(PhidiIcons::BREADCRUMB_SEPARATOR)
                                })
                                .style(move |s| {
                                    let config = config.get();
                                    let size = config.ui.icon_size() as f32;
...
fn search_editor_view(
    ...
    stack((
        ...
        clickable_icon(
            || PhidiIcons::SEARCH_CASE_SENSITIVE,
            move || {
                ...
                clickable_icon(
                    move || {
                        if replace_active.get() {
                            PhidiIcons::ITEM_OPENED
                        } else {
                            PhidiIcons::ITEM_CLOSED
                        }
                    },
                    move || {
    ...
    changes.iter().map(move |change| {
        ...
        let color = match change {
            DiffLines::Left(_range) => {
                Some(config.color(PhidiColor::SOURCE_CONTROL_REMOVED))
            }
            DiffLines::Right(_range) => {
                ...
                if modified {
                    Some(config.color(PhidiColor::SOURCE_CONTROL_MODIFIED))
                } else {
                    Some(config.color(PhidiColor::SOURCE_CONTROL_ADDED))
                }
            }
            _ => None,
        };
