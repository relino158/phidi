...
mod tests {
    ...
    use crate::{config::PhidiConfig, workspace::PhidiWorkspace};

    #[test]
    fn test_resolve() {
        // Mimicking load
        let workspace = PhidiWorkspace::default();

        let config = PhidiConfig::merge_config(&workspace, None, None);
        let mut phidi_config: PhidiConfig = config.try_deserialize().unwrap();
...
"##;
        ...
        phidi_config.available_color_themes =
            [("test".to_string(), ("test".to_string(), test_theme_cfg))]
                .into_iter()
                .collect();
        // phidi_config.available_icon_themes = Some(vec![]);
        phidi_config.core.color_theme = "test".to_string();
